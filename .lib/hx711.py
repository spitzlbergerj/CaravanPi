...
class HX711:
	...
	def __init__(self, dout, pd_sck, gain=128):
		self.error_status = False  # Initialisiere den Fehlerstatus als False
	...
	def set_gain(self, gain):
		...
		# Read out a set of raw bytes
		raw_bytes = self.readRawBytes()

		# Überprüfen, ob ein Fehler aufgetreten ist
		if raw_bytes == [None, None, None]:
			self.error_status = True  # Ein Fehler ist aufgetreten
	...
	def readRawBytes(self):
		...
		# Wait until HX711 is ready for us to read a sample. Max 5 Seconds
		while not self.is_ready():
			if time.time() - start_time > 5:  # Überprüfen, ob 5 Sekunden vergangen sind
				print("Timeout: Sensor nicht bereit")
				return [None, None, None]  # Beendet die Funktion nach 5 Sekunden
			time.sleep(0.1)  # Eine kleine Pause, um das ständige Polling zu reduzieren
