...
def write2file(chip_id, device, temperature, pressure, humidity):
	try:
		...
		file.write("\n"+ str_from_time_now + " " + strTemperature + " " + strPressure + " " + strHumidity)
		file.close()
		return 0
	except:
		# Schreibfehler
...
def main():
  ...
  if len(sys.argv) == 1: # keine weiteren Argumente -> address = 0x76 und schreiben in ein File
    DEVICE = DEVICE1
  ...
  elif len(sys.argv) == 2 or len(sys.argv) == 3: # address gemaess Parameter
    if sys.argv[1] == "76":
      DEVICE = DEVICE1
    elif sys.argv[1] == "77":
      DEVICE = DEVICE2
